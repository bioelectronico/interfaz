...
#define VSIZE 10000.0f

#define TEXREPEAT 10.0f
...
void render(float w, float h)
{
	...
	matModelView[13] = -7000.0f; 
	matModelView[14] = -5.0f -(zoom *VSIZE); //z
	...
	matProj[10] = (far + near)/(near - far);
	matProj[11] = -1.0f;
	matProj[14] = ( 2.0 * far * near ) / ( near - far );
